...
设计原则是明确划分边界：内核侧的机制与可观测的完成事实留在 API 层面，调度策略与组合逻辑由上层负责。
...
`uring` 公开面向内核的机制；调度、重试、连接追踪和协议解析属于上层职责。以下模式展示了构建上层的常见方式。
...
Multishot 操作产生 CQE 流，直到内核发送最终 CQE（不含 `IORING_CQE_F_MORE`）。框架层负责追踪订阅状态并管理重新提交。
...
| 客户端 | `TCP4Socket`, `Connect`, `Send`, `Receive` | `socket_integration_test.go` |
...
每次提交后，可复用“Ring 生命周期”一节中的 `Wait` 循环来等待完成事件。包级 `socket_integration_test.go` 覆盖了 connect/send
流程。
...
`NewZCRXReceiver` 要求 ring 以 32 字节 CQE（`IORING_SETUP_CQE32`）创建。当前 `Options` 尚未暴露该 setup flag，因此通过标准
`New` 创建的 ring 会使该构造器返回 `ErrNotSupported`。

### 生命周期

1. 在 32 字节 CQE 的 ring 上调用 `NewZCRXReceiver` 创建接收器。构造器会注册 ZCRX 接口队列、映射 refill 区域并准备 refill
   ring。
...
- `timeout_test.go`，超时与链式超时操作

包级 `listener_example_test.go` 演示监听器创建与 multishot accept，`socket_integration_test.go` 演示 TCP 客户端
connect/send 流程。
